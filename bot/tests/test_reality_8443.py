...
TEST_HOST = '198.51.100.1'  # Test IP (RFC 5737 documentation range)
...
class TestStandaloneReality8443(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = Mock()
        self.config.REALITY_HOST = TEST_HOST
        ...
        self.config.BYPASS_HOST = TEST_HOST
    ...
    def test_reality_8443_host_and_port(self):
        ...
        config_url = self.manager.generate_standalone_reality_8443(test_uuid)
        self.assertIn(f'@{TEST_HOST}:8443', config_url)
    ...
    def test_returns_none_for_placeholder_host(self):
        """Should return None when host is still the placeholder value."""
        self.config.REALITY_HOST = 'YOUR_SERVER_IP'
        config_url = self.manager.generate_standalone_reality_8443('test-uuid')
        self.assertIsNone(config_url)

    def test_returns_none_for_placeholder_key(self):
        """Should return None when public key is still the placeholder value."""
        self.config.REALITY_PUBLIC_KEY = 'YOUR_PUBLIC_KEY'
        config_url = self.manager.generate_standalone_reality_8443('test-uuid')
        self.assertIsNone(config_url)
...
class TestConfigConstants(unittest.TestCase):
    ...
    def test_reality_constants_exist(self):
        # Patch BOT_TOKEN to avoid ValueError on import
        with patch.dict(os.environ, {'BOT_TOKEN': 'test'}):
            # Need to reimport since config.py runs at import time
            import importlib
